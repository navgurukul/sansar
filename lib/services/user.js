...
const { verifyPassword } = require('../helpers/middleware');
...
module.exports = class UserService extends Schmervice.Service {
  ...
  async updateById(id, payload) {
    const { User, C4caRole ,TeacherCapacityBuilding } = this.server.models();
    try {
      let userData;
      ...
      if (payload.c4ca_partner_id){
        const rolesToCheck = ['c4caTeacher', 'c4caPartner', 'facilitator','superAdmin'];
      ...
      if (payload.name) {
        await TeacherCapacityBuilding.query()
          .where('user_id', id)
          .patch({ teacher_name: payload.name });
      }
  ...
  async loginWithGoogle(authDetails, txn) {
    ...
    const validate = this.validateIdToken(idToken);
    if (!validate) {
      return validate;
    }
  ...
  async loginMerakiStudent(user_name, password) {
    const { User } = this.server.models();
    try {
      const meraki_student = await User.query().where('user_name', user_name).first();
      if(meraki_student){
        const storedEncryptedPassword = { encryptedData: meraki_student.password, iv: meraki_student.pass_iv, tag: meraki_student.auth_tag};
        const flag = verifyPassword(password, storedEncryptedPassword);
        if(flag){
          delete meraki_student.password;
          return [null, meraki_student];
        }else{
          const error = {
            error: true,
            message: "The password does not match the username. Please enter the correct password.",
            code: 200,
            errorCode: 2002
          };
          return [error, null];
  ...
  createTokenV2 = (data) => {
    const JWTtoken = JWT.sign({ id: data.id, username: data.user_name }, CONFIG.auth.jwt.secret, {
      algorithm: 'HS256',
      expiresIn: CONFIG.auth.jwt.expiresIn,
    });
  ...
  async getUsersByDate(start_date, end_date) {
    const { User } = this.server.models();
    try {
      const users = await User.query().where('created_at', '>=', `${start_date}`).andWhere('created_at', '<', `${end_date}`);
      return [null, users];
    } catch (error) {
      return [errorHandler(error), null];
    } 
